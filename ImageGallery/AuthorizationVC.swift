...
class AuthorizationVC: UIViewController ,UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    func signUp(){
        ...
        let task = session.dataTask(with: (httpClient.postUser(email: emailTextField.text!, password: passwordTextField.text!, username: "", image: photoButton.currentImage!)) as URLRequest, completionHandler: { (data, response, error) -> Void in
